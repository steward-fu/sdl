...
#include "SDL_video.h"
#include "SDL_mouse.h"
#include "SDL_ppvideo.h"
#include "SDL_ppevent.h"
...
static int PinePhone_Available(void)
{
    debug("%s\n", __func__);
    return 1;
}

static void PinePhone_DeleteDevice(SDL_VideoDevice* device)
{
    debug("%s\n", __func__);
    if (thread_run) {
        thread_run = 0;
...
static int PinePhone_HWAccelBlit(SDL_Surface* src, SDL_Rect* srcrect, SDL_Surface* dst, SDL_Rect* dstrect)
{
    debug("%s\n", __func__);
    return 0;
}

static int PinePhone_CheckHWBlit(_THIS, SDL_Surface* src, SDL_Surface* dst)
{
    debug("%s\n", __func__);
    src->map->hw_blit = PinePhone_HWAccelBlit;
    return 1;
}

static SDL_Rect** PinePhone_ListModes(_THIS, SDL_PixelFormat* format, Uint32 flags)
{
    debug("%s\n", __func__);
    return (SDL_Rect**)-1;
}

static int PinePhone_VideoInit(_THIS, SDL_PixelFormat* vformat)
{
    debug("%s\n", __func__);
    thread_run = 1;
...
static SDL_Surface* PinePhone_SetVideoMode(_THIS, SDL_Surface* current, int width, int height, int bpp, Uint32 flags)
{
    debug("%s\n", __func__);
...
static int PinePhone_AllocHWSurface(_THIS, SDL_Surface* surface)
{
    debug("%s\n", __func__);
    return 0;
}

static void PinePhone_FreeHWSurface(_THIS, SDL_Surface* surface)
{
    debug("%s\n", __func__);
}

static int PinePhone_LockHWSurface(_THIS, SDL_Surface* surface)
{
    return 0;
}

static void PinePhone_UnlockHWSurface(_THIS, SDL_Surface* surface)
{
}

static int PinePhone_FlipHWSurface(_THIS, SDL_Surface* surface)
{
    if (wl.init && wl.ready) {
        wl.flip ^= 1;
...
static void PinePhone_UpdateRects(_THIS, int numrects, SDL_Rect* rects)
{
    PinePhone_FlipHWSurface(NULL, NULL);
}

static int PinePhone_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color* colors)
{
    debug("%s\n", __func__);
    return 0;
}

static void PinePhone_VideoQuit(_THIS)
{
    debug("%s\n", __func__);
}

static SDL_VideoDevice* PinePhone_CreateDevice(int devindex)
{
    ...
    device->VideoInit = PinePhone_VideoInit;
    device->ListModes = PinePhone_ListModes;
    device->SetVideoMode = PinePhone_SetVideoMode;
    device->CreateYUVOverlay = NULL;
    device->SetColors = PinePhone_SetColors;
    device->UpdateRects = PinePhone_UpdateRects;
    device->VideoQuit = PinePhone_VideoQuit;
    device->AllocHWSurface = PinePhone_AllocHWSurface;
    device->CheckHWBlit = PinePhone_CheckHWBlit;
    device->FillHWRect = NULL;
    device->SetHWColorKey = NULL;
    device->SetHWAlpha = NULL;
    device->LockHWSurface = PinePhone_LockHWSurface;
    device->UnlockHWSurface = PinePhone_UnlockHWSurface;
    device->FlipHWSurface = PinePhone_FlipHWSurface;
    device->FreeHWSurface = PinePhone_FreeHWSurface;
    device->SetCaption = NULL;
    device->SetIcon = NULL;
    device->IconifyWindow = NULL;
    device->GrabInput = NULL;
    device->GetWMInfo = NULL;
    device->InitOSKeymap = PinePhone_InitOSKeymap;
    device->PumpEvents = PinePhone_PumpEvents;
    device->free = PinePhone_DeleteDevice;
...
VideoBootStrap PinePhone_bootstrap = {"pinephone", "SDL PinePhone video driver", PinePhone_Available, PinePhone_CreateDevice};
