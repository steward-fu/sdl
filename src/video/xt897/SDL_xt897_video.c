...
#include "SDL_video.h"
#include "SDL_mouse.h"
#include "SDL_xt897_video.h"
#include "SDL_xt897_event.h"
...
static int XT897_Available(void)
{
    debug("%s\n", __func__);
    return 1;
}
...
static int XT897_HWAccelBlit(SDL_Surface* src, SDL_Rect* srcrect, SDL_Surface* dst, SDL_Rect* dstrect)
{
    debug("%s\n", __func__);
    return 0;
}

static int XT897_CheckHWBlit(_THIS, SDL_Surface* src, SDL_Surface* dst)
{
    debug("%s\n", __func__);
    src->map->hw_blit = XT897_HWAccelBlit;
    return 1;
}

static SDL_Rect** XT897_ListModes(_THIS, SDL_PixelFormat* format, Uint32 flags)
{
    debug("%s\n", __func__);
    return (SDL_Rect**)-1;
}

static int XT897_VideoInit(_THIS, SDL_PixelFormat* vformat)
{
    debug("%s\n", __func__);
    thread_run = 1;
...
static SDL_Surface* XT897_SetVideoMode(_THIS, SDL_Surface* current, int width, int height, int bpp, Uint32 flags)
{
    debug("%s\n", __func__);
...
static int XT897_AllocHWSurface(_THIS, SDL_Surface* surface)
{
    debug("%s\n", __func__);
    return 0;
}

static void XT897_FreeHWSurface(_THIS, SDL_Surface* surface)
{
    debug("%s\n", __func__);
}

static int XT897_LockHWSurface(_THIS, SDL_Surface* surface)
{
    return 0;
}

static void XT897_UnlockHWSurface(_THIS, SDL_Surface* surface)
{
}

static int XT897_FlipHWSurface(_THIS, SDL_Surface* surface)
{
    if (wl.init && wl.ready) {
        wl.flip ^= 1;
...
static void XT897_UpdateRects(_THIS, int numrects, SDL_Rect* rects)
{
    XT897_FlipHWSurface(NULL, NULL);
}

static int XT897_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color* colors)
{
    debug("%s\n", __func__);
    return 0;
}
...
static SDL_VideoDevice* XT897_CreateDevice(int devindex)
{
    ...
    device->ListModes = XT897_ListModes;
    device->SetVideoMode = XT897_SetVideoMode;
    device->CreateYUVOverlay = NULL;
    device->SetColors = XT897_SetColors;
    device->UpdateRects = XT897_UpdateRects;
    device->VideoQuit = XT897_VideoQuit;
    device->AllocHWSurface = XT897_AllocHWSurface;
    device->CheckHWBlit = XT897_CheckHWBlit;
    device->FillHWRect = NULL;
    device->SetHWColorKey = NULL;
    device->SetHWAlpha = NULL;
    device->LockHWSurface = XT897_LockHWSurface;
    device->UnlockHWSurface = XT897_UnlockHWSurface;
    device->FlipHWSurface = XT897_FlipHWSurface;
    device->FreeHWSurface = XT897_FreeHWSurface;
    device->SetCaption = NULL;
    device->SetIcon = NULL;
    device->IconifyWindow = NULL;
    device->GrabInput = NULL;
    device->GetWMInfo = NULL;
    device->InitOSKeymap = XT897_InitOSKeymap;
...
VideoBootStrap XT897_bootstrap = {"xt897", "SDL XT897 video driver", XT897_Available, XT897_CreateDevice};
