...
static int QX1050_Available(void)
{
    debug("%s\n", __func__);
    return 1;
}

static void QX1050_DeleteDevice(SDL_VideoDevice *device)
{
    debug("%s\n", __func__);
    if(thread_run){
        thread_run = 0;
...
static int QX1050_HWAccelBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
    debug("%s\n", __func__);
    return 0;
}

static int QX1050_CheckHWBlit(_THIS, SDL_Surface *src, SDL_Surface *dst)
{
    debug("%s\n", __func__);
    src->map->hw_blit = QX1050_HWAccelBlit;
    return 1;
}

static SDL_Rect **QX1050_ListModes(_THIS, SDL_PixelFormat *format, Uint32 flags)
{
    debug("%s\n", __func__);
    return (SDL_Rect **)-1;
}

static int QX1050_VideoInit(_THIS, SDL_PixelFormat *vformat)
{
    debug("%s\n", __func__);
    thread_run = 1;
...
static SDL_Surface *QX1050_SetVideoMode(_THIS, SDL_Surface *current, int width, int height, int bpp, Uint32 flags)
{
    debug("%s\n", __func__);
...
static int QX1050_AllocHWSurface(_THIS, SDL_Surface *surface)
{
    debug("%s\n", __func__);
    return 0;
}

static void QX1050_FreeHWSurface(_THIS, SDL_Surface *surface)
{
    debug("%s\n", __func__);
}

static int QX1050_LockHWSurface(_THIS, SDL_Surface *surface)
{
    return 0;
}

static void QX1050_UnlockHWSurface(_THIS, SDL_Surface *surface)
{
}

static int QX1050_FlipHWSurface(_THIS, SDL_Surface *surface)
{
    if(wl.init && wl.ready){
        wl.flip^= 1;
...
static void QX1050_UpdateRects(_THIS, int numrects, SDL_Rect *rects)
{
    if(wl.init && wl.ready){
        debug("%s\n", __func__);
        wl.flip^= 1;
...
static int QX1050_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors)
{
    debug("%s\n", __func__);
    return 0;
}

static void QX1050_VideoQuit(_THIS)
{
    debug("%s\n", __func__);
}

static SDL_VideoDevice *QX1050_CreateDevice(int devindex)
{
    ...
    device->VideoInit = QX1050_VideoInit;
    device->ListModes = QX1050_ListModes;
    device->SetVideoMode = QX1050_SetVideoMode;
    device->CreateYUVOverlay = NULL;
    device->SetColors = QX1050_SetColors;
    device->UpdateRects = QX1050_UpdateRects;
    device->VideoQuit = QX1050_VideoQuit;
    device->AllocHWSurface = QX1050_AllocHWSurface;
    device->CheckHWBlit = QX1050_CheckHWBlit;
    device->FillHWRect = NULL;
    device->SetHWColorKey = NULL;
    device->SetHWAlpha = NULL;
    device->LockHWSurface = QX1050_LockHWSurface;
    device->UnlockHWSurface = QX1050_UnlockHWSurface;
    device->FlipHWSurface = QX1050_FlipHWSurface;
    device->FreeHWSurface = QX1050_FreeHWSurface;
    device->SetCaption = NULL;
    device->SetIcon = NULL;
    device->IconifyWindow = NULL;
    device->GrabInput = NULL;
    device->GetWMInfo = NULL;
    device->InitOSKeymap = QX1050_InitOSKeymap;
    device->PumpEvents = QX1050_PumpEvents;
    device->free = QX1050_DeleteDevice;
    return device;
}

VideoBootStrap QX1050_bootstrap = {"qx1050", "SDL QX1050 video driver", QX1050_Available, QX1050_CreateDevice};
